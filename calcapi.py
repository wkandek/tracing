...
tracer = trace.get_tracer("calcapi")
...
def p_expression_binop(p):
  ...
  try:
    if p[2] == '+'  : 
      with tracer.start_as_current_span(module+"-plusinternal") as child:
        trace.get_current_span().set_attribute("request.id", requestid)
        trace.get_current_span().set_attribute("p1", p[1])
        trace.get_current_span().set_attribute("p2", p[3])
        if p[1] > 100:
          time.sleep(1/2)
        p[0] = p[1] + p[3]
    elif p[2] == '-': 
      with tracer.start_as_current_span(module+"-minusinternal") as child:
        trace.get_current_span().set_attribute("request.id", requestid)
        p[0] = p[1] - p[3]
    elif p[2] == '*': 
...
class CalcAPIServer(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
      ...
      # create new span with same trace id
      print(self.headers)
      ...
      #print(carrier)
      #print(ctx)
      with tracer.start_as_current_span(module+"-GetTracedAPI", context=ctx) as child:
        print(child.context[0])
        print(hex(child.context[0]))
      # carry request id through
      if "X-requestid" in self.headers:
        requestid = self.headers["x-requestid"]
        trace.get_current_span().set_attribute("request.id", requestid)
...
if __name__ == "__main__":
    starttime = time.time()
    webServer = HTTPServer((hostName, serverPort), CalcAPIServer)
    print("CalcAPIServer started http://%s:%s" % (hostName, serverPort))
    parser = yacc.yacc()
